...
impl<'a, G: Gene, GaState, IndState> Context<'a, G, GaState, IndState> {
    pub fn new(epoch: &'a Epoch<'a, GaState>, pools: &'a Pools<G, IndState>) -> Self {
        Self { epoch, pools, __: PhantomData }
    }
}

impl<'a, G: Gene, GaState, IndState> Deref for Context<'a, G, GaState, IndState> {
    type Target = Epoch<'a, GaState>;
    fn deref(&self) -> &Self::Target { self.epoch }
}
