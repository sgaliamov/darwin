...
mod tests {
    ...
    fn mutant_with_zero_noise_returns_same_genome() {
        ...
        let epoch = darwin::Epoch { generation: 0, stagnation: 0.0, normal: rand_distr::Normal::new(0.0_f32, 0.01).unwrap(), state: &none };
    ...
    fn mutant_never_exceeds_range() {
        ...
        let pools = darwin::Pools::<i64, ()>::from_vec(vec![]);
        let none: Option<()> = None;
        let epoch = darwin::Epoch { generation: 0, stagnation: 0.0, normal: rand_distr::Normal::new(0.0_f32, 1000.0_f32).unwrap(), state: &none };
        let ctx = Context::new(&epoch, &pools);
        let ind = Individual::firstborn(0, 0, genome.clone());
